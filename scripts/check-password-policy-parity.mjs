// Guards src/lib/local/password-policy.ts against drift between the two repos.
//
// Both projects set passwords on the same `users` row. The web refused `123456`
// while this API accepted it, so an account could be signed up strong on the web
// and immediately weakened through the mobile API — the floor held on whichever
// door a guest happened to use, which is the same as not holding. So this file
// must be byte-identical, not merely equivalent.
//
//   node quickin-backend/scripts/check-password-policy-parity.mjs
//
...
const FILES = {
  backend: new URL('../src/lib/local/password-policy.ts', import.meta.url),
  frontend: new URL('../../../quickin-frontend/src/lib/local/password-policy.ts', import.meta.url),
}
...
try {
  ...
  if (be === fe) {
    ...
    console.log(`✅ password-policy is identical in both repos — ${exports.length} exports (${hash(be)})`)
    console.log(`   ${exports.join(', ')}`)
    process.exit(0)
  }

  console.error('❌ password-policy.ts has DRIFTED between the two repos')
