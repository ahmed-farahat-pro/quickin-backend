// Guards src/lib/local/name-policy.ts against drift between the two repos.
//
// Both projects create accounts in the same `users` table and both decide what
// counts as a name — at signup and on a host application. If the mobile API
// accepted `12345` while the web refused it, the rule would only hold on
// whichever door a guest happened to use, which is the same as not holding. So
// this file must be byte-identical, not merely equivalent.
//
//   node quickin-backend/scripts/check-name-policy-parity.mjs
//
...
const FILES = {
  backend: new URL('../src/lib/local/name-policy.ts', import.meta.url),
  frontend: new URL('../../../quickin-frontend/src/lib/local/name-policy.ts', import.meta.url),
}
...
try {
  ...
  if (be === fe) {
    ...
    console.log(`✅ name-policy is identical in both repos — ${exports.length} exports (${hash(be)})`)
    console.log(`   ${exports.join(', ')}`)
    process.exit(0)
  }

  console.error('❌ name-policy.ts has DRIFTED between the two repos')
